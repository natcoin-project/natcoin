Natcoin Core version 0.13.2 is now available from:

  <https://download.natcoin.org/natcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/natcoin-project/natcoin/issues>
...
updates will be released anymore. Without security updates, using a natcoin
...
ECDSA signatures inside Natcoin transactions now use validation using
...
Previous versions of Natcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Natcoin Core was to raise the default minimum
relay fee.

Natcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Natcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running natcoind without having to do any manual configuration.
...
Natcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Natcoin Core will relay transactions with
...
entirely in Natcoin Core 0.13, and it is at this time undecided whether the
...
Natcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Natcoin Core automatically creates a hidden service to listen on, without
manual configuration. Natcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Natcoin Core is listening, and
...
Natcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Natcoin
Core to use floating fees. By default, Natcoin Core will use floating
fees.
...
At all times, Natcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Natcoin Core will never create transactions paying less than
...
`natcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /natcoinrpc>
        ...
        # optional bypass natcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< natcoinrpc:<password>
    </Location>
...
Restarting natcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in natcoin.conf
...
For example:
    ...
    $ src/natcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Natcoin Core code base has
...
- `natcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `natcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Natcoin scripting system that in combination with
...
Natcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Natcoin Core.
...
The two phases are distinct in the Natcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Natcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Natcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Natcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of natcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of natcoins being spent is exactly
...
- **Increased security for multisig:** Natcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Natcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Natcoin's current ledger (called the UTXO set),
  ...
  note that Natcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Natcoin users to individually opt-in to almost any change in the Natcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Natcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Natcoin
client to Natcoin Core 0.13.2 to signal for segwit support. When a
...
Since Natcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `natcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `natcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
